...
class JobRecommender(IRecommender):
    ...
    This class handles both training and prediction logic for job recommendations.
    It can be used by the Training Service (for fitting) and the ML API (for inference).
    """
    ...
    def load_artifacts(self, path: str = None):
        """Loads model artifacts from a local path."""
        target_path = path or self.artifact_path
    ...
    def recommend(self, user_skills: str, top_n: int = 5) -> Tuple[List[Dict[str, Any]], List[str]]:
        """Generates job recommendations based on user skills."""
